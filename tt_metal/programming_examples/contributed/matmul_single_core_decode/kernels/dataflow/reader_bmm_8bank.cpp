...
void kernel_main() {
    ...
    uint32_t src0_addr  = get_arg_val<uint32_t>(0) + 1;
        ...
        for (uint32_t mt = 0; mt < Mt; mt++) {
            ...
            for (uint32_t nt = 0; nt < Nt; nt++) {
                for (uint32_t kt = 0; kt < Kt; kt++) {
                    ...
                    DPRINT  << TSLICE(tt::CB::c_in0, 0, SliceRange::hw0_32_16()) << ENDL();

                    // Print a full tile
                    for (uint16_t r = 0; r < 32; ++r) {
                        SliceRange sr = SliceRange{.h0 = r, .h1 = static_cast<uint16_t>(((int)r) + 1), .hs = 1, .w0 = 0, .w1 = 32, .ws = 1};
